...
const StatCard = memo(function StatCard({
  ...
  return (
    ...
    >
      ...
      {/* Bottom Progress */}
      <div className="relative z-10 mt-auto pt-3 md:pt-5">
        <div className="mb-2 flex items-center justify-between">
          <span className="text-[10px] md:text-[11px] font-semibold uppercase tracking-wider text-gray-500">
            Active
          </span>
        ...
        <div className="h-1.5 md:h-2 w-full overflow-hidden rounded-full bg-white/80 shadow-inner">
          <div
            className={`
              h-full
              rounded-full
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mt-4 md:mt-6">
        <p className="text-xs md:text-sm font-bold tracking-wide text-gray-700">
          {card.title}
        </p>
        ...
        <p className="mt-1.5 md:mt-2 text-xs font-medium text-gray-500">
          {card.subtext}
        </p>
      </div>
...
export default function Overview({
  ...
  useEffect(() => {
    ...
    const loadStats = async () => {
      try {
        ...
        const students = safeGetData(studentsRes, []);
        const staff = safeGetData(staffRes, []);
        const vehicles = safeGetData(vehiclesRes, []);
        const vendors = safeGetData(vendorsRes, []);
        ...
        const collected = financeData.fees?.collected || 0;
        const pending = financeData.fees?.pending || 0;
  ...
  const cards: StatCardData[] = useMemo(
    () => [
      ...
      },
      {
        ...
        trend: stats.totalVendors > 0 ? '+8%' : '0%',
        trendUp: true,
      },
    ],
    [stats]
  ...
  const feeCollectionRate = useMemo(() => {
    ...
    return denom > 0
      ? Math.round((stats.totalRevenue / denom) * 100)
      : 0;
  }, [stats.totalRevenue, stats.pendingFees]);
      ...
      <div className="px-4 md:px-6 pb-4 md:pb-6 pt-2 space-y-5 animate-pulse">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {Array.from({ length: 3 }).map((_, index) => (
            <div
              key={index}
              className="h-[200px] md:h-[230px] rounded-2xl bg-white/80"
            />
          ))}
        </div>
        <div className="h-[200px] rounded-2xl bg-white/80" />
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          <div className="h-[200px] md:h-[230px] rounded-2xl bg-white/80" />
          <div className="h-[200px] md:h-[230px] rounded-2xl bg-white/80" />
          <div className="h-[200px] md:h-[230px] rounded-2xl bg-white/80" />
        </div>
    ...
    return (
      <div className="flex min-h-[400px] items-center justify-center px-4 md:px-6 py-4 md:py-6">
        <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-xl">
          ...
          <p className="mb-6 text-sm leading-6 text-gray-600">{error}</p>
  ...
  return (
    <div className="px-4 md:px-6 pb-4 md:pb-6 pt-2 space-y-4 md:space-y-5">
      ...
      <div className="rounded-2xl border border-gray-200/70 bg-white p-4 md:p-5 shadow-sm">
        <SectionHeader
          icon={Target}
          title="Quick Actions"
      ...
      <div className="grid grid-cols-1 items-stretch gap-4 lg:grid-cols-3">
        ...
        {/* Performance */}
        <div className="flex min-h-[200px] md:min-h-[230px] flex-col rounded-2xl border border-gray-200/70 bg-white p-4 md:p-5 shadow-sm">
          <SectionHeader
            icon={Gauge}
            title="Performance"
            gradient="from-green-500 to-emerald-500"
          ...
          <div className="flex flex-1 flex-col justify-between space-y-3 md:space-y-4">
            {metrics.map((metric) => (
              <div key={metric.label} className="space-y-1.5">
                ...
                <div className="h-1.5 md:h-2 w-full overflow-hidden rounded-full bg-gray-100">
                  <div
                    className={`
                      h-full
                      rounded-full
      ...
      <div className="rounded-2xl border border-gray-200/70 bg-white p-3 md:p-4 shadow-sm">
        <div className="flex items-center gap-3 rounded-xl border border-yellow-100 bg-gradient-to-r from-yellow-50 via-orange-50 to-yellow-50 p-3 md:p-4">
          {/* Award Icon */}
          <div className="flex h-10 w-10 md:h-11 md:w-11 flex-shrink-0 items-center justify-center rounded-xl bg-white shadow-sm">
            <Award className="text-yellow-600" size={20} strokeWidth={2.2} />
          </div>
          ...
          {/* Star */}
          <div className="flex h-8 w-8 md:h-9 md:w-9 flex-shrink-0 items-center justify-center rounded-full bg-white/80">
            <Star
